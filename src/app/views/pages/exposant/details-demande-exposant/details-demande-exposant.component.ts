...
@Component({
  selector: 'app-details-demande-exposant',
  templateUrl: './details-demande-exposant.component.html',
  styleUrls: ['./details-demande-exposant.component.scss']
})
export class DetailsDemandeExposantComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  this.sielService.checkUpdateExibitor(this.user.user)
    .subscribe(exhibData=>{
     this.refExposant=exhibData.refExhibitor
 ...
 getAllDataInformations(refExhibitor){
   ...
   this.sielService.getForeigenRepresented(refExhibitor).subscribe((response)=>{
    this.ForeigenRepresented = response;
    this.isLoaded=this.isLoaded+1;
  });
