...
@Component({
  selector: 'app-demande-cooperative-account',
  templateUrl: './demande-cooperative-account.component.html',
  styleUrls: ['./demande-cooperative-account.component.scss']
})

export class DemandeCooperativeAccountComponent implements OnInit {
  data:GenericPageable=new GenericPageable();
  refName="refCooperativeAccount";
  fields:string[]=["refCooperativeAccount","cooperativeName"]
  /* Static columns 
  ...
  constructor(private modalService:NgbModal,private translate:TranslateService,private handleRequestService:HandleRequestService,private cooperativeAccountService:CooperativeAccountService,public languageService: LanguageService,private router:Router) { }
  ...
  {
    this.isLoad=true;
    this.cooperativeAccountService.getCooperativeAccountsForUserWithPageAndSize(page,this.currentSize).subscribe(response=>{
        this.data=response;
    },err=>{
  ...
  updateItem(refToUpdate,index)
  {
    ...
    this.router.navigate([`/proposal-project/cooperative-account/${refToUpdate}`]);
  }
  ...
  deleteItem(componant,refToDelete,index){
    this.modalService.open(componant, {centered: true}).result.then((result) => {
      ...
      this.doingAction=true;
      this.doingActionTo=index;
      this.cooperativeAccountService.deleteCooperativeicAccount(refToDelete)
      .subscribe(e=>{
