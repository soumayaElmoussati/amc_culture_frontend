...
@Component({
  selector: 'app-demands-artist-accounts',
  templateUrl: './demands-artist-accounts.component.html',
  styleUrls: ['./demands-artist-accounts.component.scss']
})
export class DemandsArtistAccountsComponent implements OnInit {
  data:GenericPageable=new GenericPageable();
  refName="refArtistAccount";
  fields:string[]=["email","artistName"]
  /* Static columns 
  ...
  constructor(private modalService:NgbModal,private translate:TranslateService,private handleRequestService:HandleRequestService,private artistAccountService:ArtistAccountService,public languageService: LanguageService,private router:Router) { }
  ...
  {
    this.isLoad=true;
    this.artistAccountService.getArtistAccountsForUserWithPageAndSize(page,this.currentSize).subscribe(response=>{
        this.data=response;
    },err=>{
  ...
  updateItem(refToUpdate,index)
  {
    ...
    this.router.navigate([`/proposal-project/artist-account/${refToUpdate}`]);
  }
  ...
  deleteItem(componant,refToDelete,index){
    this.modalService.open(componant, {centered: true}).result.then((result) => {
      ...
      this.doingAction=true;
      this.doingActionTo=index;
      this.artistAccountService.deleteArtisticAccount(refToDelete)
      .subscribe(e=>{
