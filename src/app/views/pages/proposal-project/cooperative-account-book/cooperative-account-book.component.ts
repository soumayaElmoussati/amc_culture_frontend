...
@Component({
  selector: 'app-cooperative-account-book',
  templateUrl: './cooperative-account-book.component.html',
  styleUrls: ['./cooperative-account-book.component.scss']
})

export class CooperativeAccountBookComponent implements OnInit {
  ...
  /*****    Submit Data     *****/
  cooperativeAccountBook:CooperativeAccountBook;
  ...
  Files = ["LEGAL_STATUS_COPY","LEGAL_DEPOSIT_RECEIPT","LAST_GENERAL_MEETING_COPY","TAX_IDENTIFIER_CERTIFICATE"];
  ...
  initForme1(){
    let x  = [
      {type:"simpleInput",data:{formControlName:"cooperativeName",label:"cooperativeName",placeHolder:"",type:"text",ngModel:this.cooperativeAccountBook.cooperativeName,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','cooperativeName')},required:true}},
      {type:"simpleInput",data:{formControlName:"responsibleName",label:"responsibleName",placeHolder:"",type:"text",ngModel:this.cooperativeAccountBook.responsibleName,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','responsibleName')},required:true}},
      {type:"phoneNumber",data:{formControlName:"phoneNumber",label:"phone",placeHolder:"",ngModel:this.cooperativeAccountBook.phoneNumber,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','phoneNumber')},required:true}},
      {type:"phoneNumber",data:{formControlName:"faxNumber",label:"fax",placeHolder:"",ngModel:this.cooperativeAccountBook.faxNumber,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','faxNumber')},required:true}},
      {type:"simpleInput",data:{formControlName:"firstName",label:"firstName",placeHolder:"",type:"text",ngModel:this.cooperativeAccountBook.firstName,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','firstName')},required:true}},
      {type:"simpleInput",data:{formControlName:"lastName",label:"lastName",placeHolder:"",type:"text",ngModel:this.cooperativeAccountBook.lastName,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','lastName')},required:true}},
      {type:"selectInput",data:{formControlName:"region",label:"region",placeHolder:"",ngModel:'cooperativeAccountBook.address.region',onChange:(v)=>{this.onChangeRegion(v)},required:true,options:'regions',value:'id',labelS:'region',getLabel:(r)=>{return r.region;},multiple:false}},
      {type:"selectInput",data:{formControlName:"city",label:"city",placeHolder:"",ngModel:'cooperativeAccountBook.address.city',onChange:(v)=>{this.onChangeVille(v)},required:true,options:'villes',value:'id',labelS:'ville',getLabel:(r)=>{return r.ville;},multiple:false}},
      {type:"simpleInput",data:{formControlName:"postalCode",label:"postalCode",placeHolder:"",type:"text",ngModel:this.cooperativeAccountBook.address.postalCode,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','address.postalCode')},required:true}},
      {type:"simpleInput",data:{formControlName:"address",label:"address",placeHolder:"",type:"text",ngModel:this.cooperativeAccountBook.address.address,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','address.address')},required:true}},
      {type:"simpleInput",data:{formControlName:"province",label:"province",placeHolder:"",type:"text",ngModel:this.cooperativeAccountBook.address.province,onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','address.province')},required:false}},
      {type:"selectInput",data:{formControlName:"refArtisticProfession",label:"refArtisticProfession",placeHolder:"",ngModel:'cooperativeAccountBook.refArtisticProfession',onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','refArtisticProfession')},required:true,options:'artisticProfession',value:'refArtisticProfession',labelS:'artisticProfession',getLabel:this.getArtisticProfession,multiple:false}},
     ];
     if(this.otherData!=null && this.otherData.GeneralInfo.length>0){
       var n = [];
        for(let i of this.otherData.GeneralInfo){
          ...
          {type:"simpleInput",data:{formControlName:i.entity,label:i.label,placeHolder:"",type:i.type,ngModel:this.cooperativeAccountBook.generalInformation[i.entity],onChange:(v)=>{this.onChangevalue(v,'cooperativeAccountBook','generalInformation.'+i.entity)},required:true}})
        }
  ...
  constructor(private filesInputsService:FilesInputsService,private activatedRoute:ActivatedRoute, public formBuilder: FormBuilder,public artisticProfessionService:ArtisticProfessionService,private translate: TranslateService,private router:Router,private modalService: NgbModal, private cooperativeAccountBookService: CooperativeAccountBookService, private sielService: SielService,public  languageService: LanguageService,private snackBar:MatSnackBar,) {
    this.cooperativeAccountBook=this.cooperativeAccountBookService.initCooperativeAccountBook();
  ...
  getinitialData = ()=>{
    ...
    //to Edit !!!!!!
    let whatToSend = this.cooperativeAccountBookService.checkIfCooperativeAccountBookExist();
    if(this.refToUpdate)
      whatToSend=this.cooperativeAccountBookService.getCooperativeAccountBookByRef(this.refToUpdate);
    ...
    .subscribe(
      (re:any)=>{
        ...
        if(re[0]!=null && re[0].refCooperativeAccountBook!=null){
          if(re[0].cooperativeMember!=null) 
              this.dataStep4 = JSON.parse(re[0].cooperativeMember);
          if(re[0].status=="VALID_SUBSCRIPTION"){
            this.router.navigate(['proposal-project/cooperative-account-book-demande']);
            return;
          }
          this.setDataWithObject('cooperativeAccountBook',re[0]);
          this.cooperativeAccountBook.refArtisticProfession=re[0].artisticProfession?.refArtisticProfession;
          this.isModification=true; 
          this.cooperativeAccountBookService.getAllDocs(re[0].refCooperativeAccountBook)
          .subscribe(r=>{
            this.initinputsStep2();
            if(r.length>0){
              for (let e of r){
  ...
  getCooperativeAccountBookFile = ():any=>{
  ...
  onSubmitPage1 = ()=>{
    if(this.validationForm1.valid){
      ...
      if(!this.isModification){
        this.cooperativeAccountBookService.createNewCooperativeAccountBook({...this.cooperativeAccountBook})
          .subscribe(
            r=>{this.setDataWithObject('cooperativeAccountBook',r);this.onDone();},
          e=>{console.log(e);this.isDisabled=false;})
      }
      else {
        this.cooperativeAccountBookService.updateCooperativeAccountBook(this.cooperativeAccountBook)
        .subscribe(
          r=>{this.onDone()},
  ...
  onSubmitPage2 = ()=>{
    ...
    var dataKey  = [];
      ...
      for(let d of dtoupdate){
        ...
        if(d.file!=null){
          httpRequests.push(this.sielService.updateDoc({refDocument:d.data.refDocument,documentType:d.documentType,refObject:this.cooperativeAccountBook.refCooperativeAccountBook,file:d.file}));
    ...
    if(dtostore.length>0){
      for(let d of dtostore){
        ...
        if(d.file!=null){
          dataKey.push(d.documentType);
          httpRequests.push(this.sielService.storeDoc({refDocument:d.data.refDocument,documentType:d.documentType,refObject:this.cooperativeAccountBook.refCooperativeAccountBook,file:d.file}));
        }
      }
    }
  ...
  onSubmitPage4 = ()=>{
    if(this.dataStep4!=null &&this.dataStep4.length>0){
      this.isDisabled=true;
      this.cooperativeAccountBook.cooperativeMember = JSON.stringify(this.dataStep4);
      this.cooperativeAccountBookService.updateCooperativeAccountBook(this.cooperativeAccountBook)
        .subscribe(
  ...
  getGeneralInformationMember(){
    this.cooperativeAccountBookService.getGeneralInformationMember(this.cooperativeAccountBook.generalInformation['refGeneralInformation'])
  ...
  AddGeneralInformationMember(){
    ...
    this.cooperativeAccountBookService.createGeneralInformationMember(this.generalMember)
  ...
  EditGeneralInformationMember(){
    this.beforSendFormwithTable();
    this.cooperativeAccountBookService.editGeneralInformationMember(this.generalMember)
    .subscribe((r:GeneralMemberResponse)=>{
        this.onAdd3Files(r.refGeneralMember,()=>{
  ...
  deleteEForm3(c,i){
    ...
    this.modalService.open(c, {centered: true}).result.then((result) => {
      if(result == "save"){
      this.cooperativeAccountBookService.deleteGeneralInformationMember(this.generalMemberArray[i].refGeneralMember)
      .subscribe(e=>{
  ...
  clearForm3(){
    ...
    this.generalMember.refParent = this.cooperativeAccountBook.generalInformation['refGeneralInformation'];
  ...
  onChangeRegion = (value)=>{
    ...
    this.cooperativeAccountBook.address.region=value;
    let city=this.cooperativeAccountBook.address.city;
    if(city!=null)
      if((this.villes.filter(v=>v.id==city)).length==0)
        this.cooperativeAccountBook.address.city=null;
  }
  onChangeVille = (value)=>{
    if(value==null || value=="") return;
    this.cooperativeAccountBook.address.city=value;
  ...
  downloadFile = (index,refDoc)=>{
    ...
    this.cooperativeAccountBookService.getDocByDocRef(refDoc).subscribe((response)=>{
  ...
  valideDemande(event){
    this.isDisabled=true;
    this.cooperativeAccountBookService.validateCooperativeAccountBook(this.cooperativeAccountBook.refCooperativeAccountBook).subscribe(data=>{
      Swal.fire( { position: 'center', title: this.translate.instant("table_add_done"), text: '', showConfirmButton: false, timer: 2000, icon: 'success' } )
    .then(()=>{
      this.page = 0;
