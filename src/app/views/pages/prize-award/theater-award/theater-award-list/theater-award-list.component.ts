...
export class TheaterAwardListComponent implements OnInit {
  ...
  constructor(private hassan2AwardService: Hassan2AwardService, private theaterAwardService: TheaterAwardService) { }
  ...
  ngOnInit(): void {
    ...
    this.hassan2AwardService.getArtistInformation(this.dataStep1A.email).subscribe((response) => {
      ...
      this.theaterAwardService.getDemandsByref(this.dataStep1A.refArtistAccount)
        .subscribe(res => {
          this.data = res;
